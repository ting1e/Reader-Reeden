...
def user_settings_s3(request):
    ...
    try:
        ...
        if missing:
            return JsonResponse({'success': False, 'error': 'S3配置缺少字段: ' + ','.join(missing)})
    except Exception as e:
        logger.warning("user_settings_s3: JSON parse error", exc_info=True)
        return JsonResponse({'success': False, 'error': 'S3配置JSON格式错误: ' + str(e)[:80]})
    setting = get_or_create_user_setting(request.user)
    setting.s3_setting = s3_raw
    setting.save()
    return JsonResponse({'success': True, 'msg': 'S3配置已保存'})
    ...
    for label, r in [('主规则', rule), ('备用规则1', rule_2), ('备用规则2', rule_3)]:
        if r:
            try:
                ...
                return JsonResponse({'success': False, 'error': '%s正则表达式错误: %s' % (label, str(e)[:80])})
    setting = get_or_create_user_setting(request.user)
...
def user_settings_password(request):
    ...
    if not new_pwd:
        return JsonResponse({'success': False, 'error': '新密码不能为空'})
    if new_pwd != confirm_pwd:
        return JsonResponse({'success': False, 'error': '两次输入的新密码不一致'})
